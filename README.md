...
- オーバーサンプリング(SMOTE)
- クラス重み付け
- Out of fold(OOF)予測; 交差検証(Cross validation: CV)時にモデルを作成しアンサンブル  

であり、それぞれについて報告する。  
不均衡データの実験ではstratified k-foldを使用しているため学習と評価データのクラス数の分布は同等であり、fold数は3で実験を行っている。

### 3.1 不均衡データに対する性能
交差検証でない、それぞれの手法の実験結果は以下のようである。  
OOFはアンサンブル手法であるので、アンサンブルでない手法であれば③と④オーバーサンプリングが、accuracy, macro f1共に高い値を示している。  
アンダーサンプリングが値低い原因として、各foldで使用してないデータが存在しており、元のモデルの性能より低くなっていると考えられる。  


| experiment                   | accuracy | macro f1 | macro minority f1 | bird f1 | deer f1 | truck f1 | 
| ---------------------------- | -------- | -------- | ----------------- | ------- | ------- | -------- | 
| ①　AE + CNN 1                   | 71       | 70       | 66                | 54      | 65      | 78       | 
| ②　AE + CNN 1 + UnderSamp       | 68       | 68       | 65                | 56      | 65      | 75       | 
| ③　AE + CNN 1 + OverSamp        | 75       | 75       | 73                | 64      | 71      | 83       | 
| ④　AE + CNN 1 + OverSamp(SMOTE) | 75       | 75       | 69                | 65      | 68      | 75       | 
| ⑤　AE + CNN 1 + ClassWeight     | 72       | 72       | 71                | 62      | 67      | 83       | 
| ⑥　AE + CNN 1 + OOF             | 78       | 78       | 73                | 62      | 74      | 84       | 
|                              | 
|                              | 


### 3.2 交差検証
交差検証した場合でも、③のオーバーサンプリングが最も高い値示した。  
交差検証の場合、①アンダーサンプリングのaccuracy、f1ともに上回っており、バギングなどしない場合は、単体モデルでは性能がやや不安定であると考えられる。  
一方、③のクラスの重み付けは、交差検証ではない場合とほぼ性能が同じであるので、安定していると考えられる。


| experiment                        | accuracy | macro f1 | 
| --------------------------------- | -------- | -------- | 
| ①　AE + CNN 1 + UnderSamp + CV       | 76       | 75       | 
| ②　AE + CNN 1 + OverSamp + CV        | 87       | 87       | 
| ③　AE + CNN 1 + OverSamp(SMOTE) + CV | 73       | 71       | 
| ④　AE + CNN 1 + ClassWeight + CV     | 72       | 71       | 
|                                   | 
|                                           | 

データ拡張を加えた場合、⑤のアンダーサンプリングや⑤オーバーサンプリングの性能が低下した。  
一方、⑧のクラス重み付けや⑦のオーバーサンプリング手法の1つであるSMOTEでは性能の向上が見られた。
不均衡データに対して、データ拡張手法をする場合、通常よりも注意深くデータを見る必要があると考えられる。

| experiment                                | accuracy | macro f1 | 
| ----------------------------------------- | -------- | -------- | 
| ⑤　AE + CNN 1 + UnderSamp + Aug 2  + CV      | 74       | 72       | 
| ⑥　AE + CNN 1 + OverSamp + Aug 2  + CV       | 79       | 78       | 
| ⑦　AE + CNN 1 + OverSamp(SMOTE) + Aug 2 + CV | 76       | 75       | 
| ⑧　AE + CNN 1 + ClassWeight+ Aug 2 + CV      | 78       | 77       | 
|                                           | 
|                                           | 


### 3.3 OOF予測適用の場合
最後にアンサンブル効果のあるOOFを各手法に加えた場合の実験結果を示す。  
①のアンダーサンプリング以外では、不均衡データ対策手法を加えてない④より性能向上が見られた。  
②のオーバーサンプリングでは、各手法で少数データのf1が60台あるのに対して、すべて70以上の値を示した。  
これらの実験から、不均衡データに対してはオーバーサンプリングとOOFが最も効果的であると考える。


| experiment                         | accuracy | macro f1 | macro minority f1 | bird f1 | deer f1 | truck f1 | 
| ---------------------------------- | -------- | -------- | ----------------- | ------- | ------- | -------- | 
| ①　AE + CNN 1 + UnderSamp + OOF       | 76       | 76       | 74                | 65      | 71      | 85       | 
| ②　AE + CNN 1 + OverSamp + OOF        | 79       | 79       | 77                | 71      | 75      | 86       | 
| ③　AE + CNN 1 + OverSamp(SMOTE) + OOF | 77       | 77       | 74                | 67      | 74      | 82       | 
| ③　AE + CNN 1 + ClassWeight + OOF     | 78       | 78       | 74                | 67      | 69      | 85       | 
| ④　AE + CNN 1 + OOF                   | 78       | 78       | 73                | 62      | 74      | 84       | 
|                                    | 
|                                           | 
...
今回の課題ではlossとaccuracyから、適切なデータ拡張はAug 2と判断した。
...
実験結果と学習サイクルを考え、適切なデータ拡張はAug 2と判断した。
...
## 5. まとめ
このレポートではcifar-10の不均衡データかつ、AEで特徴量抽出を行った場合、以下のことを検討した。
1. オートエンコーダ(以下AE)のモデル
2. AEから特徴量を抽出した後に連結した畳み込み分類モデル(以下CNN)
3. 不均衡データに対する施策
4. データ拡張手法
...
実験の結果、モデルの性能向上のため
1. オートエンコーダ(以下AE)のモデル=>Stack AEではなく単一AEを使用
2. AEから特徴量を抽出した後に連結した畳み込み分類モデル(以下CNN)=>12層のモデル
3. 不均衡データに対する施策=>オーバーサンプリングとOOFを併用
4. データ拡張手法=>Aug 2のデータ拡張手法を使用

が適切であると判断した。

最後にこれらの適切な選択を適用した場合と初期のモデルの実験結果を報告する。  
モデルのチューニングや、データ拡張、不均衡データに対する施策などを行うことにより、初期のモデルの性能を大きく上回ることができ、不均衡データのクラスに対するrecallも向上した。  
損失関数での対応や、データ拡張ではあまり効果が出せなったので、今後はそのあたりを検証したい。
...
| ------------------------------------ | -------- | -------- | ----------------- | ------- | ------- | -------- | 
| AE + CNN 1                           | 71       | 70       | 66                | 54      | 65      | 78       | 
| AE + CNN 2 + OverSamp + Aug 2  + OOF | 88       | 88       | 86                | 81      | 86      | 92       | 
|                                      | 
|                                      | 
